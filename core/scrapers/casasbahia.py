...
def extrair_casasbahia(url):
    print("⚡ [Casas Bahia] Extração limpa via Undetected ChromeDriver...")
    url_limpa = url.split("?")[0]
    
    # Delay de 8s porque o site deles é um pouco mais pesado
    ...
    if matches:
        for match in matches:
            try:
                ...
                if val > 20.0:  
                    preco_atual = val
                    print(f"🔍 [DEBUG CASAS BAHIA] Preço capturado do HTML limpo: R$ {preco_atual}")
                    break
            except:
                continue
                
    if not preco_atual:
        print("❌ [Casas Bahia] Preço não encontrado no HTML.")
        return None
        
    # 📸 Recuperando a foto oficial em alta qualidade
    ...
    tag_h1 = soup.find("h1")
    nome_produto = tag_h1.text.strip() if tag_h1 else (titulo.split("-")[0].strip() if titulo else "Produto Casas Bahia")
